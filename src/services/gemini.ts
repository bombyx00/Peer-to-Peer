...
const generateMockComprehensiveFeedback = (
  ...
  let selfCommentSummary = selfTextComment;
  if (selfCommentSummary.length > 30) {
    selfCommentSummary = selfCommentSummary.substring(0, 30) + '...';
  }
...
/**
 * Gemini API를 사용해 여러 동료 평가와 자기평가를 종합한 AI 종합 피드백 문장을 작성합니다.
 */
export const generateComprehensiveAIFeedback = async (
  projectName: string,
  questions: { id: string; questionText: string; type: string }[],
  evaluateeName: string,
  allEvaluations: any[],
  ...
  if (!isGeminiConfigured()) {
    console.log('Gemini API 키가 없어 로컬 룰에 의한 종합 피드백을 생성합니다.');
